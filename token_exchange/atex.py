...
class ATEX:
 ...
 return {"ok": True, "account": acc_id, "balance": credit}
 ...
 """结算：仅owner可用，将平台佣金Token转入owner账户"""
 acc = self.get_account(acc_id)
 if not acc:
 return {"ok": False, "err": "account_not_found"}
 ...
 acc["balance"] += amount
 self._save()
 self._log(f"settle:{acc_id},amount:{amount},remaining_commission:{self.ob['total_commission_earned']}")
 return {
 "ok": True, "settled": amount,
 "owner_balance": acc["balance"],
