...
import json, os, sys, time, threading, hashlib, uuid as _uuid_mod
...
def _load_saas():
 ...
 return {"users": {}, "access_keys": {}, "usage": []}
...
class Handler(BaseHTTPRequestHandler):
 ...
 auth = self.headers.get("Authorization", "").replace("Bearer ", "")
 user = _saas_user(auth) if auth else None
 if not user: return self._json({"err": "invalid_access_key", "message": "Invalid API key. Get one at http://localhost:8420"}, 401)
 ...
 data = _load_saas()
 uid = f"u_{_uuid_mod.uuid4().hex[:12](6)}"
 access_key = f"atex_ak_{_uuid_mod.uuid4().hex[:48]}"
 data["users"][uid] = {"user_id": uid, "name": name, "email": email,
 "access_key": access_key, "balance_cny": 0.0, "total_spent_cny": 0.0, "total_calls": 0,
 "created": datetime.now(TZ).strftime("%Y-%m-%d %H:%M:%S")}
 data["access_keys"][access_key] = uid
 _save_saas(data)
 self._json({"ok": True, "user_id": uid, "access_key": access_key, "balance_cny": 0.0,
 "note": "Top up at http://localhost:8420 to start using APIs"})
 ...
 if deploy_token != "CHANGE_ME_DEPLOY_TOKEN":
